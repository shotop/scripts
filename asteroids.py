...
def click(pos):
    global started, timer, lives, score, soundtrack
...
def process_sprite_group(sprite_set, canvas):
    ...
    for sprite in list(sprite_set):
        if sprite.update():
            ...
            sprite.draw(canvas)
    sprite_set.difference_update(temp)
...
def group_collide(group, other_object):
    ...
    for sprite in list(group):
        if sprite.collide(other_object):
            rs.add(sprite)
            collision += 1
            expl = Sprite(sprite.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
